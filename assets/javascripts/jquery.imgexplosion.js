...
(function($, window, document, undefined) {
    ...
    imgExplosion.prototype.init = function() {
      ...
      var centerOn = $(this.opt.centerOn)
      this.opt.inPlace = true;
      this.opt.centerLeft = centerOn.offset().left;
      this.opt.centerWidth = centerOn.width();
      this.opt.centerTop = centerOn.offset().top;
      this.opt.centerHeight = centerOn.height();
    }
    ...
    var x = null;
    if (o.centerOn) {
      x = this.randBetween(o.centerLeft, o.centerLeft + o.centerWidth);
      o.targetX = Math.floor($(window).scrollLeft() + 
                             ($(window).width() - this.imgW - o.extraWidth) * (x - o.centerLeft) / o.centerWidth);
    } else {
    ...
    var y = null;
    if (o.centerOn) {
      y = this.randBetween(this.opt.centerTop, this.opt.centerTop + this.opt.centerHeight);
      o.targetY = Math.floor($(window).scrollTop() + 
                             ($(window).height() - this.imgH - o.extraWidth) * (y - o.centerTop) / o.centerHeight);
    } else {
			y = o.inPlace ? this.randBetween(this.diam,this.maxOffset) : this.diam-this.imgW;
    }
    //console.log("x:"+x+" y:"+y + " target x: " + o.targetX + " target y: " + o.targetY);
      ...
      if (typeof o.targetX !== "undefined" && o.targetX !== null) {
        anim.left = o.targetX;
      } else {
        anim.left = this.randBetween(x, this.dw - this.imgW);
      }

      if (typeof o.targetY !== "undefined" && o.targetY !== null) {
        anim.top = o.targetY;
      } else {
